...
export function normalizeCurrency(input: string | null | undefined): string {
  const code = String(input || "INR").toUpperCase();
...
export function currencyFromLocale(locale: string | null | undefined): string {
  ...
  return LOCALE_TO_CURRENCY[locale] || "INR";
}
...
export function formatCurrency(amount: number, currency: string | null | undefined, locale = "en-IN"): string {
  const normalized = normalizeCurrency(currency);
