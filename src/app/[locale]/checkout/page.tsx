...
export default function CheckoutPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      const currency = normalizeCurrency( "INR");
...
baseCurrency: "INR",
      ...
      if (selectedGateway === "razorpay") {
        ...
        const razorpay = new window.Razorpay({
         ...
         amount: Math.round(Number(data?.totals?.total || discountedTotal) * 100),
          currency,
          name: "Koda Store",
          ...
          {step === "payment" && (
            <div className="animate-in">
              ...
              <button className="btn btn-primary btn-lg" style={{ width: "100%", marginTop: "1.5rem", height: "3.25rem" }} onClick={handleCheckout} disabled={loading || availableGateways.length === 0}>
                {loading
                  ? "Processing Payment..."
                  : `Pay ${(discountedTotal)} with ${availableGateways.find((gateway) => gateway.id === selectedGateway)?.name || "Gateway"}`}
              </button>
          ...
          <div style={{ borderTop: "1px solid var(--outline-variant)", paddingTop: "1rem", marginTop: "0.5rem" }}>
            ...
            {appliedCoupon && (
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.875rem", marginBottom: "0.5rem" }}>
                ...
                <span style={{ color: "#166534", fontWeight: 700 }}>-{(appliedCoupon.discountAmount)}</span>
              </div>
            )}
          </div>
