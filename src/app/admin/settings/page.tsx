...
export default function AdminSettings() {
  ...
  // shiprocket
  const [shiprocketEmail, setShiprocketEmail] =
  useState("");

const [shiprocketPassword, setShiprocketPassword] =
  useState("");

const [savingShiprocket, setSavingShiprocket] =
  useState(false);

const [shiprocketSaved, setShiprocketSaved] =
  useState(false);

  const [shiprocketEditMode, setShiprocketEditMode] =
  useState(false);

  const [showShiprocketPassword, setShowShiprocketPassword] =
  useState(false);
...
if (shiprocketRes.ok) {
  const shiprocketData =
    await shiprocketRes.json();

  setShiprocketEmail(
    shiprocketData.email || ""
  );

  setShiprocketPassword(
    shiprocketData.password || ""
  );
}
      } catch (loadError) {
  ...
  const handleShiprocketSave = async () => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        data.error ||
          "Failed to save Shiprocket settings"
      );
    }

    setShiprocketSaved(true);
setShiprocketEditMode(false);

    setTimeout(() => {
      setShiprocketSaved(false);
    }, 2500);
  } catch (error: any) {
